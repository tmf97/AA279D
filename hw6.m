...
% mean AOE
maoe_chief_i = [10000000, 0.01, 0.1, 0.1, 0.1, 0.1];
a0 = maoe_chief_i(1);

mean_roes_initial = [0.1, 0.1, 100, 100, 0.1, 0.1].' / a0; 
mean_roes_desired = [0.3, 0.3, 10, 20, 0.1, 0.1].' / a0;
...
figure
hold on 
plot(tspan/Tp, dt*delta_vs(1,:), 'DisplayName','x')
plot(tspan/Tp, dt*delta_vs(2,:), 'DisplayName','y')
plot(tspan/Tp, dt*delta_vs(3,:), 'DisplayName','z')
xlabel('Orbit Number')
ylabel('Delta V (m/s)')
hold off
legend
...
subplot(1,3,1)
hold on
plot(a0*mean_roes(2, :), a0*mean_roes(1, :))
scatter(a0*mean_roes_desired(2), a0*mean_roes_desired(1))
scatter(a0*mean_roes(2, 1), a0*mean_roes(1, 1));
