...
initial_offsets = [50000; -50000; 50000; 100; 100; 100];
...
for i=2:length(ts)
    ...
    % FIXME: sign error on the Kalman Filter (3) slide??
    state_plus = state_minus + K*(measurement - measurement_model(state_minus, measurement_noise));
...
subplot(4,2,7);
hold on
plot(plot_t, vecnorm(estimated_states_chief(1:3, :) - truth_states_chief(1:3, :)), "DisplayName", "True Position Error")
plot(plot_t, vecnorm(P_mags(1:3,:)).^0.5, "DisplayName", "Covariance Magnitude")
...
subplot(4,2,8);
hold on
plot(plot_t, vecnorm(estimated_states_chief(4:6, :) - truth_states_chief(4:6, :)), "DisplayName", "True Velocity Error")
plot(plot_t, vecnorm(P_mags(4:6,:)).^0.5, "DisplayName", "Covariance Magnitude")
